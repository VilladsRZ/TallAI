...
for i in range(num_classes):
    if not os.path.exists(f"data/{i}"):
        os.mkdir(f"data/{i}")
for i in range(num_classes):
    if not os.path.exists(f"testdata/{i}"):
        os.mkdir(f"testdata/{i}")
...
def generate_image(data_name,N):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
        ...
        futures = [executor.submit(lambda:img_gen(N*i,data_name,N)) for i in range(num_threads)]
...
def img_gen(start_index,data_name,N):
    ...
    for index,i in enumerate(images):
        ...
        for n in i:
            ...
            draw.line([(n[0][0],n[0][1]), (n[1][0],n[1][1])], fill='black', width=ran_int)
        file_path = f'{data_name}/{int(i[0][1][2])}/{index+start_index}.png'
        print("Image data: ", img)
        img.save(f'{data_name}/{int(i[0][1][2])}/{index+start_index}.png',mode="L")
        print(f"Image saved: {os.path.exists(file_path)}")  # This will print True if the file exists
